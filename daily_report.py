...
def _extract_summary_from_md(md_path: str) -> str:
    ...
    从MD文件中提取 [Nav_Summary] 行作为summary


    严格遵循「4月6日架构」Nav_Summary 前置规范


    该函数先检查 [Nav_Summary] 前置字段，若无则通读第一段正文
    ...
    try:
        ...
        for line in lines:
            ...
            # 优先匹配 [Nav_Summary] 前置字段（4月6日架构强制要求）


            if line.startswith('[Nav_Summary]'):


                c = line[len('[Nav_Summary]'):].strip()
...
def update_website_manifest(new_entry: dict):
    ...
    """将新日报元数据追加/更新到网站的日报列表中（已存在条目则更新summary）"""
    ...
    # ── 
    # ─── 更新已存在条目（而非跳过）：确保 [Nav_Summary] 实时刷新 ───

    import re as _re

    date_block_pat = _re.compile(

        r"    \{[^}]*?date:\s*[\'\"]" + _re.escape(target_date) + r"[\'\"][^}]*?\},\s*\n",

        _re.DOTALL

    )

    if date_block_pat.search(content):

        md_path2 = os.path.join(REPORTS_DIR, new_entry["activeFile"])

        real_summary = _extract_summary_from_md(md_path2)

        badge_map = {"high": "badge-red", "medium": "badge-yellow", "low": "badge-green"}

        level_map = {"high": "高风险", "medium": "中高风险", "low": "中等风险"}

        new_entry["summary"] = real_summary

        new_entry["badge"] = badge_map.get(new_entry.get("panicLevel", "medium"), "badge-yellow")

        new_entry["panicLevelText"] = level_map.get(new_entry.get("panicLevel", "medium"), "中高风险")
        ...
        nb = (
            ...
            pi=new_entry["panicIndex"],

            pl=new_entry["panicLevel"],

            plt=new_entry["panicLevelText"],

            af=new_entry["activeFile"]

        )

        content = date_block_pat.sub(nb, content, count=1)

        with open(index_path, "w", encoding="utf-8") as f:

            f.write(content)

        print("  [网站] 已更新 " + new_entry["displayDate"] + " summary: " + real_summary)
